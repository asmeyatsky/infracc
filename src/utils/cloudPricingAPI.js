 ...
 * 
 * Uses real pricing APIs when available, falls back to mock data
 */

import CloudPricingIntegration from './realPricingAPI';

// Mock pricing data - fallback when real APIs are unavailable
...
class CloudPricingAPI {
  ...
  static async getGCPPrices(serviceType, region = 'us-central1') {
    // Try to use real pricing API first
    try {
      const availability = await CloudPricingIntegration.checkAvailability();
      if (availability.gcp) {
        switch (serviceType) {
          case 'computeEngine':
            return await CloudPricingIntegration.getPricing('gcp', 'compute', {
              machineType: 'n1-standard-1',
              region,
            });
          case 'cloudStorage':
            return await CloudPricingIntegration.getPricing('gcp', 'storage', {
      ...
      console.warn('Real GCP pricing API unavailable, using mock data:', error);
    }
